...
import { OverviewRoadmapSection } from "../../components/overview/OverviewRoadmapSection";
import { StatePanel } from "../../components/state/StatePanel";
import { OnboardingChecklist } from "../../components/onboarding/OnboardingChecklist";
import { EmptyState } from "../../components/ui/EmptyState";
import { OverviewBoardChart } from "../../components/overview/OverviewBoardChart";
import { OverviewProgressRing } from "../../components/overview/OverviewProgressRing";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[240px_1fr]">
        <div className="surface-panel flex flex-col items-center justify-center p-8">
          <OverviewProgressRing
            percent={progress?.percent ?? 0}
            done={progress?.done ?? 0}
            total={progress?.total ?? 0}
            active={progress?.active ?? 0}
            label={t("overview.roadmap")}
            phasesDoneLabel={t("overview.phasesDone", {
              done: progress?.done ?? 0,
              total: progress?.total ?? 0,
            })}
            inProgressLabel={
          ...
          {QUICK_LINKS.map((item, i) => (
            ...
            >
              <Link
                ...
                className="group surface-card block h-full p-4 no-underline hover:-translate-y-1 hover:shadow-[var(--shadow-card-hover)]"
              >
                <p className="m-0 font-medium text-[var(--text-primary)] transition group-hover:text-[var(--accent)]">
                  {t(item.labelKey)}
                </p>
                <p className="m-0 mt-1 text-xs leading-relaxed text-[var(--text-muted)]">{t(item.descKey)}</p>
                <span className="mt-3 inline-block text-xs text-[var(--accent)] opacity-0 transition group-hover:opacity-100">
                  →
                </span>
              </Link>
            </motion.div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        <div className="space-y-6">
          <OverviewRoadmapSection
            slug={slug}
            enrichedPhases={roadmapData?.enriched_phases}
            phaseDepths={roadmapData?.phase_depths}
            mermaid={roadmapData?.mermaid}
          />
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="surface-panel p-6">
              ...
              <OverviewBoardChart items={boardCounts} />
            </section>
